...
function isIdDigit(inputValue) {
  if (inputValue.length != 16 && inputValue.length != 12 && inputValue.length < 7) {
    return false;
  }
  return true;
}
...
function checkEmail(email) {
  ...
  if (!isEmailFormat(email)) {
    errorMsg.innerHTML = "Mohon isi email yang benar.\nContoh: foo@example.com atau bar@example.nom.za";
    errorMsg.style.display = "block";
    return false;
  }
...
function checkTelp(telp) {
  ...
  if (!isTelpFormat(telp)) {
    errorMsg.innerHTML = "Mohon isi nomor telepon dengan benar.\nContoh: +62 XXXXXXXXXX, +62-XXX-XXXX-XXXX, XXXX-XXXX-XXXX, XXXXXXXXXXXX";
    errorMsg.style.display = "block";
    return false;
  }
...
function checkId(idNum) {
  ...
  if (!isNumeric(idNum)) {
    errorMsg.innerHTML = "Mohon isi nomor ID dengan benar.";
    errorMsg.style.display = "block";
    return false;
  }
  if (!isIdDigit(idNum)) {
